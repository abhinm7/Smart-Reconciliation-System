  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl text-gray-700">Reconciliation Dashboard</h1>
          <p className="text-gray-500">
            Manage your system data and Transaction uploads
          </p>
        </div>
              ...
              jobs.map((job) => (
                <tr key={job._id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <span
                      className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full uppercase ${getStatusBadge(job.status)}`}
                    >
                      {job.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    job.type === "RECONCILIATION" ? (
                      <Link
                        ...
                        className="text-indigo-600 hover:text-indigo-900 font-bold border border-indigo-200 px-3 py-1 rounded hover:bg-indigo-50"
                      >
                        View Results →
                      </Link>
                    ) : job.status.toLowerCase() === "completed" &&
